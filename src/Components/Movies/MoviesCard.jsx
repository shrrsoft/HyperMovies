...
export default function MoviesCard({movie}) {
  ...
  return (
    ...
    <div className='w-fit bg-cover relative m-5 text-slate-300 bg-gradient-to-t from-slate-900 via-slate-700 to-slate-900 mx-auto rounded-lg p-1.5 ' >
        <div onMouseMove={(e)=>SetOpacityImg('opacity-20')}  onMouseLeave={(e) => SetOpacityImg("")}>
        <img className={`h-80  transition-all mx-auto delay-50 duration-500 ${opacityImg}`} src={(posterImage(movie.poster_path))} alt="" />
        <button className={` top-32 left-12 ${opacityImg ? 'absolute ' : 'hidden'}`}>
            <span className='text-white bg-gradient-to-br from-rose-900 to-rose-500 px-6 py-2.5 rounded-xl flex'  href="#">Read More</span>
            </button>
        </div>
